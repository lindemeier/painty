...
DigitalPaintMainWindow::DigitalPaintMainWindow(QWidget* parent)
    : QMainWindow(parent),
      ui(new Ui::DigitalPaintMainWindow) {
  ui->setupUi(this);
  ...
  connect(Rw, SIGNAL(currentColorChanged(const QColor&)), this,
          SLOT(setPaintingColorRw()));

  connect(ui->rb_slider, SIGNAL(valueChanged(int)), this,
          SLOT(setPaintingColorRw()));
  connect(ui->brushRadiusSlider, SIGNAL(valueChanged(int)),
          m_canvasView->getDigitalCanvas(), SLOT(setBrushRadius(int)));
  connect(ui->dryCanvasButton, SIGNAL(pressed()),
          m_canvasView->getDigitalCanvas(), SLOT(dryCanvas()));

  m_canvasView->getDigitalCanvas()->setBrushRadius(
    ui->brushRadiusSlider->value());
  setPaintingColorRw();
  ...
  connect(aboutAct, &QAction::triggered, [=]() {
    QMessageBox::about(this, tr("About Application"),
                       tr("Created by:\t\tThomas "
                          "Lindemeier\nDate:\t\t2017\n\nEmail:\t\tthomas."
                          "lindemeier@gmail.com\n\nSimple program "
                          "demonstrating the creation of brush strokes."));
  });

  ui->spinBoxPickupRate->setValue(
    m_canvasView->getDigitalCanvas()->getFootprintBrushPtr()->getPickupRate());
  ui->spinBoxDepositionRate->setValue(m_canvasView->getDigitalCanvas()
                                        ->getFootprintBrushPtr()
                                        ->getDepositionRate());
  connect(
    ui->spinBoxPickupRate, QOverload<double>::of(&QDoubleSpinBox::valueChanged),
    [=](double v) {
      m_canvasView->getDigitalCanvas()->getFootprintBrushPtr()->setPickupRate(
        v);
    });
  connect(ui->spinBoxDepositionRate,
          QOverload<double>::of(&QDoubleSpinBox::valueChanged), [=](double v) {
            m_canvasView->getDigitalCanvas()
              ->getFootprintBrushPtr()
              ->setDepositionRate(v);
          });

  ui->spinBoxDryingTime->setValue(static_cast<double>(
    m_canvasView->getDigitalCanvas()->getCanvas()->getDryingTime().count()));
  connect(ui->spinBoxDryingTime,
          QOverload<double>::of(&QDoubleSpinBox::valueChanged), [=](double v) {
            m_canvasView->getDigitalCanvas()->getCanvas()->setDryingTime(
              std::chrono::milliseconds(static_cast<uint32_t>(v * 1000.0)));
          });

  ui->checkBoxUseSnapshotBuffer->setChecked(m_canvasView->getDigitalCanvas()
                                              ->getFootprintBrushPtr()
                                              ->getUseSnapshotBuffer());
  connect(ui->checkBoxUseSnapshotBuffer,
          QOverload<int>::of(&QCheckBox::stateChanged), [=](int state) {
            const auto checked = (state == Qt::Checked);
            m_canvasView->getDigitalCanvas()
              ->getFootprintBrushPtr()
              ->setUseSnapshotBuffer(checked);
          });

  connect(ui->radioButtonFootprintBrush,
          QOverload<bool>::of(&QRadioButton::clicked), [=](bool clicked) {
            m_canvasView->getDigitalCanvas()->setUseFootprintBrush(clicked);
          });
  connect(ui->radioButtonTextureBrush,
          QOverload<bool>::of(&QRadioButton::clicked), [=](bool clicked) {
            m_canvasView->getDigitalCanvas()->setUseFootprintBrush(!clicked);
          });
}
...
void DigitalPaintMainWindow::setPaintingColorRw() {
  ...
  const auto clamp = [](int32_t x, int32_t min, int32_t max) {
    return std::min(max, std::max(x, min));
  };

  QColor Rwc_clamped(clamp(rw.red(), 2, 254), clamp(rw.green(), 2, 254),
                     clamp(rw.blue(), 2, 254));
  QColor Rbc_clamped(clamp(rb.red(), 1, Rwc_clamped.red() - 1),
                     clamp(rb.green(), 1, Rwc_clamped.green() - 1),
