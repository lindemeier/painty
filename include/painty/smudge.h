...
public:
  Smudge(const uint32_t size) : _maxSize(size), _pickupMapSrc(_maxSize, _maxSize), _pickupMapDst(_maxSize, _maxSize)
  {
    if ((_maxSize % 2) == 0)
    {
      _maxSize++;
  ...
  void smudge(Canvas<vector_type>& canvas, const vec2& boundMin,
              SplineEval<std::vector<vec2>::const_iterator>& spineSpline, T length, const Mat<T>& thicknessMap)
  {
    ...
    for (double u = 0.0; u < 1.0; u += 1. / length)
    {
      ...
      bresenham(rasteredPoints, static_cast<int32_t>(p0[0]), static_cast<int32_t>(p0[1]), static_cast<int32_t>(p1[0]),
    ...
    for (size_t i = 0; i < rasteredPoints.size(); ++i)
    {
      ...
      for (auto x = 0U; x < _pickupMapDst.getCols(); x++)
      {
        ...
        {
          if (x < 0U || x >= _pickupMapDst.getCols() || y < 0U || y >= _pickupMapDst.getRows())
            continue;
          vec<int32_t, 2> sp(x, y);
          vec<int32_t, 2> cp(x + roi_x, y + roi_y);
          vec<int32_t, 2> tp(static_cast<int32_t>(cp[0] - boundMin[0U]), static_cast<int32_t>(cp[1] - boundMin[1U]));

          const auto dist = (center - cp.cast<double>()).norm();
          if (dist > radius)
            continue;

          const auto D = thicknessMap(static_cast<uint32_t>(tp[1]), static_cast<uint32_t>(tp[0]));
          if (D <= 0.0)
          {
            continue;
          }
          ...
          const auto cV =
              canvas.getPaintLayer().getV_buffer()(static_cast<uint32_t>(cp[1]), static_cast<uint32_t>(cp[0]));
          const auto pV = _pickupMapDst.getV_buffer()(static_cast<uint32_t>(sp[1]), static_cast<uint32_t>(sp[0]));
          ...
          const auto canvasK =
              canvas.getPaintLayer().getK_buffer()(static_cast<uint32_t>(cp[1]), static_cast<uint32_t>(cp[0]));
          const auto canvasS =
              canvas.getPaintLayer().getS_buffer()(static_cast<uint32_t>(cp[1]), static_cast<uint32_t>(cp[0]));

          const auto pickK = _pickupMapDst.getK_buffer()(static_cast<uint32_t>(sp[1]), static_cast<uint32_t>(sp[0]));
          const auto pickS = _pickupMapDst.getS_buffer()(static_cast<uint32_t>(sp[1]), static_cast<uint32_t>(sp[0]));
          ...
          if (pVnew > 0.0)
          {
            ...
            _pickupMapDst.getK_buffer()(static_cast<uint32_t>(sp[1]), static_cast<uint32_t>(sp[0])) =
                pVnew_ * (pVr * pickK + cVl * canvasK);
            _pickupMapDst.getS_buffer()(static_cast<uint32_t>(sp[1]), static_cast<uint32_t>(sp[0])) =
                pVnew_ * (pVr * pickS + cVl * canvasS);
            _pickupMapDst.getV_buffer()(static_cast<uint32_t>(sp[1]), static_cast<uint32_t>(sp[0])) =
                std::max(pVnew, 0.0);
          }
...
private:
  ...
  {
    const auto theta = std::atan2(heading[1], heading[0]);  // get rotation around tool
    ...
    for (auto x = 0U; x < _maxSize; x++)
    {
      for (auto y = 0U; y < _maxSize; y++)
      {
        ...
        if (pickupPos[0] < 0 || pickupPos[1] < 0 || pickupPos[0] >= _maxSize || pickupPos[1] >= _maxSize)
        {
  ...
  void bresenham(std::vector<vec<int32_t, 2UL>>& points, int32_t x0, int32_t y0, int32_t x1, int32_t y1)
  {
    /**
     * @author Zingl Alois
     * @date 22.08.2016
